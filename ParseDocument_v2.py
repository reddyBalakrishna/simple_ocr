...
class Document():
    ...
    def dilateImage(self, image, number):
        kernel = np.ones((18,number),np.uint8) #number : represents wideness of the dilation. i.e for line level or word level or character level.
    ...
    def getCountours(self,input_Image):
        ...
        contours,h= cv2.findContours(input_Image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    def getTextFromImage(self, image, contours, Width=5, Height=5):
        ...
        Word_Dilated_Image = Document.dilateImage(self,image,9)
    ...
    def storeData(self, data):
        ...
        try:
            ...
            df.to_csv(self.path + 'Output/Output.txt',index=False)
            '''
            Output = open(self.path + 'Output/Output.txt', 'w')
            data=str(data)
            Output.write(data)
            Output.close()
